...
const LoginPage: React.FC = () => {
  ...
  const handle = (e: React.FormEvent) => {
    ...
    if(isLogin){
      ...
      if(ok) navigate('/account'); else setError('Invalid credentials. Please try again.')
    } else {
      if(!form.name||!form.email||!form.phone||!form.password){setError('Please fill all fields.');return}
      signup(form.name,form.email,form.phone,form.password); navigate('/account')
    }
  }
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-3xl shadow-xl border border-gold-100 overflow-hidden">
          ...
          <form onSubmit={handle} className="p-6 md:p-8 space-y-4">
            ...
            {!isLogin&&<div><label className="font-body text-sm font-medium text-gray-700 mb-1.5 block">Phone Number</label><input type="tel" value={form.phone} onChange={e=>setForm(f=>({...f,phone:e.target.value}))} placeholder="+91-XXXXXXXXXX" className="input-field"/></div>}
            <div>
