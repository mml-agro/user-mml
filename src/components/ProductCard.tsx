      ...
      <div className="p-4 md:p-5 flex flex-col flex-1">
        ...
        <div className="flex gap-1.5 mb-4 mt-auto">
          {(['1L', '5L', '15L'] as const).map(s => (
            <button key={s} onClick={() => setSize(s)} className={`flex-1 py-1.5 rounded-lg text-xs font-body font-semibold border-2 transition-all ${size === s ? 'border-gold-500 bg-gold-50 text-gold-700' : 'border-gray-200 text-gray-500 hover:border-gold-300'}`}>{s}</button>
          ))}
        </div>
